import { formatNobleName, NOBLE_TILE_SIZE } from '../../constants/theme';
...
export function NobleTile({ noble }: NobleTileProps) {
  return (
    <div
      className={[
        'shrink-0 rounded-lg border border-purple-500/50 bg-gradient-to-br from-purple-900/80 to-slate-800 p-2 shadow-md',
    ...
    >
      ...
      <div className="flex flex-wrap gap-0.5">
        {REGULAR_GEM_TYPES.filter((gem) => (noble.requirements[gem] ?? 0) > 0).map(
          (gem) => (
            <GemCountBadge
              key={gem}
              gem={gem}
